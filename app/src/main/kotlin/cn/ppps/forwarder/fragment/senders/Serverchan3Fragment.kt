...
import cn.ppps.forwarder.databinding.FragmentSendersServerchan3Binding
...
class Serverchan3Fragment : BaseFragment<FragmentSendersServerchan3Binding?>(), View.OnClickListener {

    private val TAG: String = Serverchan3Fragment::class.java.simpleName
    ...
    override fun viewBindingInflate(
        ...
        container: ViewGroup,
    ): FragmentSendersServerchan3Binding {
        return FragmentSendersServerchan3Binding.inflate(inflater, container, false)
    }

    override fun initTitle(): TitleBar? {
        titleBar = super.initTitle()!!.setImmersive(false).setTitle(R.string.server_chan3)
        return titleBar
    }
        ...
        Core.sender.get(senderId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(object : SingleObserver<Sender> {
            ...
            override fun onSuccess(sender: Sender) {
                ...
                if (settingVo != null) {
                    ...
                    binding!!.etTags.setText(settingVo.tags)
                    binding!!.etShort.setText(settingVo.short)
                }
            }
        })
    }
                ...
                R.id.btn_test -> {
                    ...
                    Thread {
                        try {
                            ...
                            Serverchan3Utils.sendMsg(settingVo, msgInfo)
                        } catch (e: Exception) {
                            e.printStackTrace()
                            Log.e(TAG, "onClick: $e")
    ...
    private fun checkSetting(): Serverchan3Setting {
        ...
        //校验 SendKey 能否推导出 Server酱³ 接口地址（支持 sctp 开头的 SendKey 或完整 API 地址）
        if (Serverchan3Utils.buildRequestUrl(sendKey).isNullOrEmpty()) {
            throw Exception(getString(R.string.invalid_server_chan3_sendkey))
        }

        val tags = binding!!.etTags.text.toString().trim()
        if (!TextUtils.isEmpty(tags)) {
            //多个标签以竖线分隔，例如：服务器报警|报告
            val regex = """^[^|]+(\|[^|]+)*$""".toRegex()
            if (!regex.matches(tags)) {
                throw Exception(getString(R.string.invalid_server_chan3_tags))
            }
        }

        return Serverchan3Setting(sendKey, tags, binding!!.etShort.text.toString().trim())
    }
